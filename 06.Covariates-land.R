...
#4. Import data----
data.raw <- read.csv("Data/CONIMCP_CleanDataAll_Habitat_Roosting_100km.csv", header = T)
...
for(i in 1:length(years)){
  ...
  for(j in 1:loops){
    ...
    task_vector <- ee_table_to_drive(collection=image.evi,
                                     description=paste0("EVI_land_",year.i,"-",j),
                                     folder="MCP",
                                     timePref=FALSE)
    task_vector$start()
#    ee_monitoring(task_vector) # optional
    ...
    task_vector <- ee_table_to_drive(collection=image.lc,
                                     description=paste0("LC_land_",year.i,"-",j),
                                     folder="MCP",
                                     timePref=FALSE)
    task_vector$start()
#    ee_monitoring(task_vector) # optional
...
#24. Write to csv----
write.csv(data.season, "Data/Covariates_land.csv", row.names=FALSE)
